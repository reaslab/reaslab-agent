import { Slug } from "@reaslab-agent/util/slug"
...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    if (!result.parentID && (Flag.REASLAB_AUTO_SHARE || cfg.share === "auto"))
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".reaslab", "plans")
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // Cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
      ...
      const total = iife(() => {
        ...
        if (
          (input.model.api as any).npm === "@ai-sdk/anthropic" ||
          (input.model.api as any).npm === "@ai-sdk/amazon-bedrock" ||
          (input.model.api as any).npm === "@ai-sdk/google-vertex/anthropic"
        ) {
      ...
      const costInfo =
        (input.model as any).cost?.experimentalOver200K && tokens.input + tokens.cache.read > 200_000
          ? (input.model as any).cost.experimentalOver200K
          : (input.model as any).cost
      return {
        cost: safe(
          new Decimal(0)
