    ...
    async execute(params: z.infer<typeof parameters>, ctx) {
      ...
      const session = await iife(async () => {
        ...
        return await Session.create({
          ...
          permission: [
            ...
            ...(((config as any).experimental?.primary_tools as string[] | undefined)?.map((t: string) => ({
              pattern: "*",
      ...
      const result = await SessionPrompt.prompt({
        ...
        tools: {
          ...
          ...Object.fromEntries(((config as any).experimental?.primary_tools as string[] ?? []).map((t: string) => [t, false])),
        },
        parts: promptParts,
      })
